...
interface CartContextType {
  ...
  totalPrice: number;
  isLoading: boolean;
  // transient UI pulse to signal cart update (e.g. animate navbar cart)
  pulse: boolean;
  triggerPulse: () => void;
}
...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const loadCart = useCallback(async () => {
    ...
    if (user && user.id) {
      ...
      try {
        ...
        console.log(
          "⚠️ Backend cart unavailable, using local storage fallback"
        );
    ...
    (cartItems: CartItem[]) => {
      if (!user) {
        try {
          ...
          // trigger a UI pulse for other components (e.g., header cart icon)
          setPulse(true);
  ...
  const addItem = useCallback(
    async (item: Omit<CartItem, "qty">, qty = 1) => {
      ...
      if (user && user.id) {
        ...
        try {
          ...
          setItems((prev) => {
            ...
            // also trigger UI pulse when adding
            setPulse(true);
            setTimeout(() => setPulse(false), 700);
            return newItems;
          });
        ...
        setItems((prev) => {
          ...
          if (existingItem) {
            newItems = prev.map((p) =>
              p.id === item.id ? { ...p, qty: Math.min(p.qty + qty, 9999) } : p
            );
          } else {
            newItems = [...prev, { ...item, qty }];
          }
          ...
          saveGuestCart(newItems);
          // trigger UI pulse when guest adds
          setPulse(true);
          setTimeout(() => setPulse(false), 700);
          return newItems;
        });
