...
const CustomerDashboard: React.FC = () => {
  const { user } = useAuth();
  const { bookings, orders, events, branches, sessions } = useData();

  const [trackOrder, setTrackOrder] = React.useState<null | (typeof orders)[0]>(
    null
  );
  ...
  const gridClasses =
    "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8";
  ...
  const downloadTicket = async (booking: any) => {
    ...
    const session =
      sessions?.find(
        (s: any) => s._id === booking.sessionId || s.id === booking.sessionId
      ) || null;
    // event may be missing or ids may not match (mock data); prefer session.eventId then booking.eventId
    const event =
    ...
    const ticketData = {
      ticketId: booking.id,
      eventTitle:
        event?.title || session?.label || booking.activity || "Booking",
      eventDescription: event?.description || session?.description || null,
      // prefer session date/time, then booking, then event
      date: session?.date || booking.date || event?.date,
      time: session?.time || booking.time || event?.time,
      duration: session?.duration || event?.duration || null,
      venue: branch?.name || null,
      address: branch?.address || null,
      // additional session metadata
      sessionLabel: session?.label || null,
      sessionType: session?.type || null,
      sessionAgeGroup: session?.ageGroup || null,
      sessionAvailableSeats:
        typeof session?.availableSeats !== "undefined"
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-6 md:mb-8 flex items-center justify-between">
          ...
          <div>
            <Link
              to="/profile"
              className="inline-flex items-center gap-2 bg-white border border-gray-200 px-3 py-2 rounded-md shadow-sm text-sm"
            >
              Edit Profile
            </Link>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-4 md:p-6 flex items-center justify-between">
            <div>
              ...
              <div className="mt-2 text-sm text-gray-700">
                ...
                <div>
                  <span className="font-medium">Customer ID:</span> {user?.id}
                </div>
                <div>
            ...
            <div className="text-sm text-gray-500">
              This information is included in your bookings and orders for
              tracking purposes.
            </div>
          </div>
        </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm text-gray-600 mb-3">
                        <div>
                          ...
                          {session?.date || booking.date || event?.date}
                        </div>
                        <div>
                          <span className="font-medium">Time:</span>{" "}
                          {session?.time || booking.time || event?.time}
                        </div>
                        <div>
                              ...
                              {order.trackingUpdates.map((update, index) => (
                                ...
                                >
                                  <div className="flex-1">
                                    <div className="flex items-center space-x-2">
                                      ...
                                      <span className="font-medium">
                                        {getOrderStatusLabel(update.status) ||
                                          update.status}
                                      </span>
                                    </div>
                                  ...
                                  <span className="text-gray-500 ml-2">
                                    {(() => {
                                      const ts =
                                        update.createdAt || update.timestamp;
                                      return ts
                                        ? new Date(
                                            ts as unknown as string
                                          ).toLocaleDateString()
                                        : "";
                                    })()}
                                  </span>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg">
            <h3 className="text-lg font-bold text-gray-900 mb-3">
              Order Tracking - #{trackOrder.id.slice(-8)}
            </h3>
                ...
                >
                  <div>
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-2 h-2 rounded-full ${
                          u.status === "delivered" || u.status === "Delivered"
                            ? "bg-green-500"
                            : u.status === "shipped" ||
                              u.status === "Shipped" ||
                      ...
                      ></div>
                      <span className="font-medium">
                        {getOrderStatusLabel(u.status) || u.status}
                      </span>
                    </div>
                  ...
                  <div className="text-xs text-gray-500">
                    {(() => {
                      ...
                      return ts
                        ? new Date(ts as unknown as string).toLocaleString()
                        : "";
                    })()}
                  </div>
                </div>
              ))}
              {(!trackOrder.trackingUpdates ||
                trackOrder.trackingUpdates.length === 0) && (
