...
const Header: React.FC = () => {
  ...
  const { totalItems, isLoading, pulse } = useCart();
  ...
  const linkBase =
    "text-black hover:text-[#6B4396] px-3 py-2 rounded-md uppercase";
  ...
  return (
    <>
      <DiscountBar />
      {/* High-z pulse overlay: renders above all pages when pulse is active */}
      {pulse && (
        <div
          aria-hidden
          className="fixed top-4 right-6 z-[99999] pointer-events-none"
        >
          <div className="animate-ping bg-green-400/40 rounded-full w-12 h-12 flex items-center justify-center shadow-xl">
      ...
      <nav
        id="universalNavbar"
        className="sticky top-0 inset-x-0 z-[2000]  shadow-sm"
          ...
          <div className="flex items-center justify-between h-16 sm:h-[86px]">
            <div className="flex items-center gap-4">
              <Link to="/" className="flex items-center no-underline">
                ...
                <span
                  className="ml-3 font-bold text-xl sm:text-2xl"
                  style={{ color: "#7F55B1" }}
                >
                  Artgram
                </span>
              ...
              <div className="relative" ref={desktopDropdownRef}>
                ...
                {desktopDropdownOpen && (
                  <div className="absolute left-0 top-full mt-2 w-72 rounded-lg bg-white shadow-xl ring-1 ring-black/5 p-2">
                    <Link
                      to="/slime-play.html"
                      className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-purple-600 hover:text-white transition-colors no-underline"
                    >
            ...
            <div className="flex items-center gap-3">
              ...
              {showCartIcon && (
                <Link
                  to="/cart"
                  className={`ml-0 sm:ml-2 inline-flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-2 rounded-md no-underline transition-all ${
                    pulse ? "ring-2 ring-green-400 bg-white/80" : ""
                  }`}
                >
                  <span
                    className={`text-lg transition-transform ${
                      pulse ? "text-green-600 scale-125" : "text-slate-900"
                    }`}
                    role="img"
                    aria-label="cart"
                  >
                  ...
                  {!isLoading && (
                    <span
                      className={`text-xs font-medium ${
                        pulse ? "text-green-600" : "text-gray-700"
                      }`}
                    >
              ...
              {!user && (
                <Link
                  to="/login"
                  className="ml-2 inline-block rounded-full text-white px-3 py-1 text-sm sm:px-4 sm:py-2 font-semibold transition-all no-underline bg-[#7F55B1] hover:bg-[#6B4396]"
                >
                  LOGIN
                </Link>
              )}
              ...
              {user && (
                <>
                  <Link
                    ...
                    onMouseEnter={(e) =>
                      (e.currentTarget.style.backgroundColor = "#6B4396")
                    }
                    onMouseLeave={(e) =>
                      (e.currentTarget.style.backgroundColor = "#7F55B1")
                    }
                  >
                    DASHBOARD
                  </Link>
              ...
              {user && (
                <>
                  <Link
                    ...
                    className={`${linkBase} block ${
                      isActive(["/admin", "/manager", "/dashboard"])
                        ? activeLink
                        : ""
                    }`}
                  >
                  ...
                  {user.role === "admin" && (
                    <div className="mt-2 pl-3 space-y-1 text-sm border-l border-purple-200">
                      <Link
                        to="/admin#sessions"
                        className="block hover:text-[#6B4396] no-underline"
                      >
                        Manage Sessions
                      </Link>
                      <Link
